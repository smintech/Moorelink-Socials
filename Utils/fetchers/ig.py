...
GRAPHQL_TIMEOUT = 15.0  # 15 seconds for GraphQL API
HTML_CAPTURE_TIMEOUT = 15.0  # 15 seconds for HTML capture
...
# GraphQL doc_ids - update as needed
PROFILE_POSTS_DOC_ID = "9310670392322965"  # User timeline
USER_INFO_DOC_ID = "2398832706970914"      # Alternative for user info if needed
...
# ══════════════════════════════════════════════
#  SCRAPER with API + HTML Fallback
# ══════════════════════════════════════════════
...
class InstagramCaptionScraper2026:
    ...
    async def _capture_html(self, page: Page) -> bytes:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                self.logger.debug(f"HTML too small, retry {attempt}", indent=2)
            except:
                self.logger.debug(f"content() failed attempt {attempt}", indent=2)
        ...
        try:
            ...
            self.logger.debug("HTML capture failed", indent=2)
            return b""
    ...
    async def safe_goto(self, page: Page, url: str) -> bool:
        try:
            ...
            try:
                if any(p in url for p in ("/p/", "/reel/", "/tv/")):
    ...
    async def _collect_post_urls_html(self, page: Page, post_limit: int) -> List[str]:
        post_urls: List[str] = []
        last_height = 0
        stale_rounds = 0
        MAX_STALE = 3
    ...
    async def scrape_profile_html(self, page: Page, username: str, post_limit: int) -> List[Dict]:
        ...
        post_urls = await self._collect_post_urls_html(page, post_limit)
        ...
        for i, url in enumerate(post_urls, 1):
            ...
            result = await self.scrape_single_post_html(page, url, shortcode, i)
            if result.success:
                posts.append(result.data)
            await self._human_delay(1200, 3000)  # Longer delay for HTML to avoid limits
        ...
        try:
            async with async_playwright() as p:
                ...
                if len(posts) >= post_limit:
                    ...
                    posts = await self.scrape_profile_html(page, username, post_limit)
